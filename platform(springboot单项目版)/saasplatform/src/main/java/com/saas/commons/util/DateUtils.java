...
public class DateUtils {
	...
	public static final String PATTERN_MONTH_NO_DAY = "yyyy-MM-00 00:00:00";
	/**
	 * 时间格式化
	 *
	 * @param date
	...
	/**
	 * 返回当前月份 @Title: getCurrentMonth @Description:
	 * TODO(这里用一句话描述这个方法的作用) @param @param date @param @param
	 * patternDate2 @param @return 设定文件 @return Date 返回类型 @throws
	 */
	public static Date getCurrentMonth(Date date) {
		SimpleDateFormat sdf = new SimpleDateFormat(PATTERN_MONTH);
		String time = sdf.format(date);
		return parse(time, PATTERN_MONTH);
	}

	public static String getCurrentMonthByString(Date date) {
		SimpleDateFormat sdf = new SimpleDateFormat(PATTERN_MONTH);
		String time = sdf.format(date);
		return time;
	}
	
	public static Integer getCurrentMonthByInt(Date date) {
